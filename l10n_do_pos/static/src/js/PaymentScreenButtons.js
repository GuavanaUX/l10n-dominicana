...
export class L10nDoPosPaymentScreen extends PaymentScreen {
        ...
        if (this.pos.config.l10n_do_fiscal_journal) {
            ...
            if (current_order.fiscal_type.requires_document && !client) {
                this.popup.add(ErrorPopup, {
                    ...
                    body: _.str.sprintf(_t('For invoice fiscal type %s its necessary customer, please select customer'), fiscal_type.name),
                });
                return false;
            }
            ...
            if (fiscal_type.requires_document && !client.vat) {
                this.popup.add(ErrorPopup, {
                    title: _t('Required document (RNC/Cedula)'),
                    body: _.str.sprintf(_t('For invoice fiscal type %s it is necessary for the customer have RNC or Cedula'), fiscal_type.name),
                });
                return false;
            }
            ...
            if (fiscal_type.requires_document && !(client.vat.length === 9 || client.vat.length === 11)) {
                this.popup.add(ErrorPopup, {
                    ...
                    body: _.str.sprintf(_t('For invoice fiscal type %s it is necessary for the customer have correct RNC or Cedula without dashes or spaces'), fiscal_type.name),
                });
                return false;
            }
            ...
            if (current_order.get_fiscal_type().prefix === 'B14') {
                ...
                if (has_taxes) {
                    this.popup.add(ErrorPopup, {
                        ...
                        body: _.str.sprintf(_t('You cannot pay order of Fiscal Type %s with ITBIS/ISC. Please select correct fiscal position for remove ITBIS and ISC'), fiscal_type.name),
                    });
                    return false;
                }
            }
        ...
        if (this.pos.config.l10n_do_fiscal_journal && paymentMethod && paymentMethod.is_credit_note) {
            ...
            if (current_partner && current_partner.id !== this.pos.config.pos_partner_id[0]) {
                try {
                    ...
                    const { confirmed: confirmedPickingCreditNote, payload: credit_note } = await this.popup.add(SelectionPopup, {
                ...
                const { confirmed, payload: ncf } = await this.popup.add(TextInputPopup, {
                    startingValue: '',
            ...
            const credit_note_partner = this.pos.db.get_partner_by_id(credit_note.partner_id);
    ...
    _updateSelectedPaymentline() {
        if (this.selectedPaymentLine && this.selectedPaymentLine.payment_method.is_credit_note && this.pos.config.l10n_do_fiscal_journal) {
            this.popup.add(ErrorPopup, {
                title: _t('Error'),
    ...
    async analyze_payment_methods() {
        ...
        let total_in_bank = 0;
        let has_cash = false;
        const payment_lines = current_order.get_paymentlines();
        const total = current_order.get_total_with_tax();

        for (const payment_line of payment_lines) {
            ...
            if (payment_line.payment_method.is_credit_note && !current_order._isRefundAndSaleOrder()) {
                if (!payment_line.credit_note_ncf) {
                    this.popup.add(ErrorPopup, {
                        ...
                        body: _t('There is an error with the payment of credit note, please delete the payment of the credit note and enter it again.'),
                    });
                    return false;
                }
                try {
                    ...
                    if (credit_note.residual_amount < payment_line.amount) {
                        this.popup.add(ErrorPopup, {
                            title: _t('Error in credit note'),
                            body: _t('The amount of the credit note is less than the amount entered, please delete the payment of the credit note and enter it again.'),
                        });
                        return false;
                    }
                } catch (error) {
        ...
        const roundedTotal = Math.round(Math.abs(total) * 100) / 100;
        const roundedBankTotal = Math.round(Math.abs(total_in_bank) * 100) / 100;

        if (roundedTotal < roundedBankTotal) {
            this.popup.add(ErrorPopup, {
        ...
        if (roundedBankTotal === roundedTotal && has_cash) {
            this.popup.add(ErrorPopup, {
                title: _t('Card and cash payment'),
                body: _t('The total payment with the card is sufficient to pay the order, please eliminate the payment in cash or reduce the amount to be paid by card'),
            });
            return false;
        }
